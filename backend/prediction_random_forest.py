...
def install_missing_libraries():
    for module_name, package_name in REQUIRED_PACKAGES.items():
        if importlib.util.find_spec(module_name) is None:
            print(f"[RF] Installing missing library: {package_name}")
...
PREDICTIONS_PATH = PROJECT_ROOT / 'data' / 'rf_predictions.csv'
...
def load_data():
    print('[RF] Loading data/bridge_features.csv')
    df = pd.read_csv(FEATURE_PATH)
    print(f"[RF] Loaded shape: {df.shape}")
    return df
...
def ensure_is_fault(df):
    if 'is_fault' not in df.columns:
        raise ValueError(
            ...
            "No fallback label derivation is allowed for this supervised model."
        )
...
def print_class_distribution(name, y):
    counts = y.value_counts().reindex([0, 1], fill_value=0).sort_index()
    percentages = (counts / len(y) * 100.0).round(1)
    print(
        f"[RF] {name} class distribution: "
...
def split_time_ordered(X, y, df):
    ...
    print(f"[RF] Train rows: {len(X_train)}")
    print(f"[RF] Test rows: {len(X_test)}")
    train_counts = print_class_distribution('Train', y_train)
    test_counts = print_class_distribution('Test', y_test)

    if y_train.nunique() < 2 or y_test.nunique() < 2:
        raise ValueError(
            "Train and test splits must both contain classes 0 and 1. "
            "Adjust the time split or source data so the test set has a balanced mix."
        )
...
def train_model(X_train, y_train, X_test):
    ...
    print_class_distribution('Train after SMOTE', pd.Series(y_train_resampled))
...
def evaluate_model(model, X_test_scaled, y_test):
    ...
    print(f"[RF] F1 Score : {f1:.3f}")
    print(f"[RF] AUC-ROC  : {auc_roc:.3f}")
    print(f"[RF] Threshold: {PREDICTION_THRESHOLD:.2f}")
...
def save_model_outputs(model, scaler):
    ...
    print('[RF] Saved → models/random_forest.pkl')
    print('[RF] Saved → models/rf_scaler.pkl')
...
def save_predictions(test_meta, y_test, y_pred, y_prob):
    ...
    print('[RF] Saved → data/rf_predictions.csv')
...
def plot_confusion_matrix(y_test, y_pred):
    ...
    fig.tight_layout()
    fig.savefig(PLOTS_DIR / '10_rf_confusion_matrix.png', dpi=150)
    plt.close(fig)
    print('[RF] Saved → plots/10_rf_confusion_matrix.png')
...
def plot_roc_curve(fpr, tpr, auc_roc):
    ...
    fig.tight_layout()
    fig.savefig(PLOTS_DIR / '11_rf_roc_curve.png', dpi=150)
    plt.close(fig)
    print('[RF] Saved → plots/11_rf_roc_curve.png')
...
def main():
    ...
    print('[RF] Random Forest prediction pipeline complete.')
